...
class Autoscript extends CI_Controller {
        ...
        if($checkBonusSet)
        {
            if($smallestPoint >= 4)
            {
                ...
                { 
                    ...
                    $limit_bonus        = $this->_check_limit_bonus($userid, $zenxBonus, $zenx_price);
                    $excess_bonus       = $zenxBonus - $limit_bonus;
    ...
    public function miningResult()
    {
        ...
        $query_zenx      = $this->M_user->get_data_bydate_user('mining', 'datecreate', 'type', $dateNow, '3')->row_array();
        $user           = $this->M_user->get_alluser_mining($dateNow)->result();

        $global_mining          = $query_global['amount'];
        $global_airdrop_zenx    = $query_zenx['amount'];
        ...
        foreach($user as $row_user)
        {
            ...
            $aidrop_zenx =  $global_airdrop_zenx * $row_user->point;
            ...
            $start_mining_zenx      = date('Y-m-d', strtotime("+1 day", strtotime($datepay)));
            $all_use_mining_zenx    = (strtotime($dateNow) - strtotime($start_mining)) / (60 * 60 * 24);

            $limit_bonus            = $this->_check_limit_bonus($user_id, $aidrop_zenx, $zenx_price);
            $excess_bonus           = $aidrop_zenx - $limit_bonus;
            $limit_count_zenx       = $limit_bonus;

        
            if($dateNow >= $start_mining_zenx)
            {
                if($global_airdrop_zenx > 0 )
                {
                    if($all_use_mining <= 540)
                    {
                        $data_airdrop = [
                            'user_id' => $user_id,
                        ...
                        $check_airdrop = $this->M_user->row_data_bydate_user('airdrop_zenx', 'datecreate', 'cart_id', $dateNow, $cart_id);
    
                        if($check_airdrop < 1)
                        {
                            $this->M_user->insert_data('airdrop_zenx', $data_airdrop);
    ...
    private function _count_bonus_binmatch($id, $user_id, $date, $generation)
    {
        ...
        $omset = $this->M_user->get_total_bydata_date('bonus_maxmatching', 'zenx', 'user_id', $user_id, $date);
                
        if(!empty($omset['zenx']))
        {
            $bonus = (20*$omset['zenx'])/100;
            $query_conversion_zenx = $this->M_user->get_last_data('market_price', 'convert', 'DESC');
            $zenx_price            = $query_conversion_zenx['convert'];

            $limit_bonus        = $this->_check_limit_bonus($user_id, $bonus, $zenx_price);
    ...
    public function bonusGlobal()
    {
        ...
        if(!empty($global_omset))
        {
            ...
            if(!empty($amount_fm4))
            {
                $bonus_fm4  = ((($global_omset * 2)/100)*$zenx_price)/$amount_fm4;
            }
    
            if(!empty($amount_fm5))
            {
                $bonus_fm5  = ((($global_omset * 1)/100)*$zenx_price)/$amount_fm5;
            }
    
            if(!empty($amount_fm6))
            {
                $bonus_fm6  = ((($global_omset * 0.5)/100)*$zenx_price)/$amount_fm6;
            }
    
            if(!empty($amount_fm7))
            {
                $bonus_fm7  = ((($global_omset * 0.4)/100)*$zenx_price)/$amount_fm7;
            }
    
            if(!empty($amount_fm8))
            {
                $bonus_fm8  = ((($global_omset * 0.3)/100)*$zenx_price)/$amount_fm8;
            }
    
            if(!empty($amount_fm9))
            {
                $bonus_fm9  = ((($global_omset * 0.2)/100)*$zenx_price)/$amount_fm9;
            }
    
            if(!empty($amount_fm10))
            {
                $bonus_fm10 = ((($global_omset * 0.1)/100)*$zenx_price)/$amount_fm10;
            }
            ...
            foreach($query_user->result() as $row_user)
            {
                if($row_user->fm == 'FM4')
                {                                                                                                                                                                                                                                             
                    $this->_insert_bonus_global($row_user->id, $bonus_fm4, $date, 'FM4', $zenx_price);
                }
                elseif($row_user->fm == 'FM5')
                {
                    $this->_insert_bonus_global($row_user->id, $bonus_fm5, $date, 'FM5', $zenx_price);
                    $this->_insert_bonus_global($row_user->id, $bonus_fm4, $date, 'FM4', $zenx_price);
                }
                elseif($row_user->fm == 'FM6')
                {
                    $this->_insert_bonus_global($row_user->id, $bonus_fm6, $date, 'FM6', $zenx_price);
                    $this->_insert_bonus_global($row_user->id, $bonus_fm5, $date, 'FM5', $zenx_price);
                    $this->_insert_bonus_global($row_user->id, $bonus_fm4, $date, 'FM4', $zenx_price);
                }
                elseif($row_user->fm == 'FM7')
                {
                    $this->_insert_bonus_global($row_user->id, $bonus_fm7, $date, 'FM7', $zenx_price);
                    $this->_insert_bonus_global($row_user->id, $bonus_fm6, $date, 'FM6', $zenx_price);
                    $this->_insert_bonus_global($row_user->id, $bonus_fm5, $date, 'FM5', $zenx_price);
                    $this->_insert_bonus_global($row_user->id, $bonus_fm4, $date, 'FM4', $zenx_price);
                }
                elseif($row_user->fm == 'FM8')
                {
                    $this->_insert_bonus_global($row_user->id, $bonus_fm8, $date, 'FM8', $zenx_price);
                    $this->_insert_bonus_global($row_user->id, $bonus_fm7, $date, 'FM7', $zenx_price);
                    $this->_insert_bonus_global($row_user->id, $bonus_fm6, $date, 'FM6', $zenx_price);
                    $this->_insert_bonus_global($row_user->id, $bonus_fm5, $date, 'FM5', $zenx_price);
                    $this->_insert_bonus_global($row_user->id, $bonus_fm4, $date, 'FM4', $zenx_price);
                ...
                elseif($row_user->fm == 'FM9')
                {
                    $this->_insert_bonus_global($row_user->id, $bonus_fm9, $date, 'FM9', $zenx_price);
                    $this->_insert_bonus_global($row_user->id, $bonus_fm8, $date, 'FM8', $zenx_price);
                    $this->_insert_bonus_global($row_user->id, $bonus_fm7, $date, 'FM7', $zenx_price);
                    $this->_insert_bonus_global($row_user->id, $bonus_fm6, $date, 'FM6', $zenx_price);
                    $this->_insert_bonus_global($row_user->id, $bonus_fm5, $date, 'FM5', $zenx_price);
                ...
                elseif($row_user->fm == 'FM10')
                {
                    $this->_insert_bonus_global($row_user->id, $bonus_fm10, $date, 'FM10', $zenx_price);
                    $this->_insert_bonus_global($row_user->id, $bonus_fm9, $date, 'FM9', $zenx_price);
                    $this->_insert_bonus_global($row_user->id, $bonus_fm8, $date, 'FM8', $zenx_price);
                    $this->_insert_bonus_global($row_user->id, $bonus_fm7, $date, 'FM7', $zenx_price);
                    $this->_insert_bonus_global($row_user->id, $bonus_fm6, $date, 'FM6', $zenx_price);
    ...
    private function _insert_bonus_global($user_id, $bonus, $date, $fm, $zenx_price)
    {
        $newdate = $date.' 00:00:00';
        $d = DateTime::createFromFormat('Y-m-d H:i:s', $newdate)->getTimestamp();
        ...
        $check_data = $this->M_user->row_check_global($user_id, $year.'-'.$month);
    ...
    public function bonusBasecamp()
    {
        ...
        $query_basecamp = $this->M_user->get_all_basecamp();
        ...
        {
            $limit_basecamp         = $this->_check_limit_bonus($row_user->id, $row_basecamp->zenx, $zenx_price);
            $excess_bonus_basecamp  = $row_basecamp->zenx - $limit_basecamp;
            $limit_count_basecamp   = $limit_basecamp;
            
            $data_update = [
                'status' => '1',
                'zenx' => $limit_count_basecamp,
                'update_date' => time()
            ];
            ...
            $data_excess_basecamp = [
                'user_id' => $row_basecamp->user_id,
                'id_bs' => $row_basecamp->id_bs,
                'type_bonus' => '3',
                'zenx' => $excess_bonus_basecamp,
