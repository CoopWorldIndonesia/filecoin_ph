...
class M_user extends CI_Model
{
    ...
    {
        return $this->db->select('bonus.datecreate, user.username, bonus.filecoin, bonus.zenx')
            ->from('bonus')
    ...
    {
        return $this->db->select('bonus_sm.datecreate, user.username, bonus_sm.filecoin, bonus_sm.zenx')
            ->from('bonus_sm')
    ...
    {
        return $this->db->select('bonus_maxmatching.id, user.username, bonus_maxmatching.zenx, bonus_maxmatching.set_amount, bonus_maxmatching.datecreate')
    ...
    {
        return $this->db->select('bonus_binarymatch.id, user.username, bonus_binarymatch.generation, bonus_binarymatch.zenx, bonus_binarymatch.datecreate')
    ...
    {
        return $this->db->select('bonus_global.id, user.username, bonus_global.zenx, bonus_global.datecreate')
            ->from('bonus_global')
    ...
    {
        return $this->db->select('bonus_basecamp.datecreate, user.username, bonus_basecamp.cart_id, bonus_basecamp.team, bonus_basecamp.filecoin, bonus_basecamp.mtm, bonus_basecamp.type')
            ->from('bonus_basecamp')
            ->join('cart', 'bonus_basecamp.cart_id = cart.id')
            ->join('user', 'cart.user_id = user.id')
            ->where('bonus_basecamp.user_id', $id)
    ...
    {
        return $this->db->select('cart.user_id, cart.fill, cart.sponsor_id, cart.matching_id, package.id AS package_id, code_bonus.code, code_bonus.amount_sp, code_bonus.amount_sm, package.fil as price')
            ->from('cart')
            ->join('package', 'cart.package_id = package.id')
    ...
    public function get_today_bonus($date, $id)
    {
        return $this->db->select('user.id, 
                                    ...
                                    (SELECT sum(bonus.zenx) FROM bonus WHERE bonus.user_id = user.id AND from_unixtime(bonus.datecreate, "%Y-%m-%d") = "' . $date . '") sponsorzenx,
                                    (SELECT sum(bonus_sm.zenx) FROM bonus_sm WHERE bonus_sm.user_id = user.id AND from_unixtime(bonus_sm.datecreate, "%Y-%m-%d") = "' . $date . '") sponmatchingzenx,
                                    (SELECT sum(bonus_maxmatching.zenx) FROM bonus_maxmatching WHERE bonus_maxmatching.user_id = user.id AND from_unixtime(bonus_maxmatching.datecreate, "%Y-%m-%d") = "' . $date . '") pairingmatch,
                                    ...
                                    (SELECT sum(bonus_binarymatch.zenx) FROM bonus_binarymatch WHERE bonus_binarymatch.user_id = user.id AND from_unixtime(bonus_binarymatch.datecreate, "%Y-%m-%d") = "' . $date . '") binarymatch,
                                    (SELECT sum(bonus_global.zenx) FROM bonus_global WHERE bonus_global.user_id = user.id AND from_unixtime(bonus_global.datecreate, "%Y-%m-%d") = "' . $date . '") bonusglobal,
                                    (SELECT sum(bonus_basecamp.zenx) FROM bonus_basecamp WHERE bonus_basecamp.user_id = user.id AND from_unixtime(bonus_basecamp.datecreate, "%Y-%m-%d") = "' . $date . '") basecampzenx')
            ->from('user')
            ->where('user.id', $id)
            ->get();
    }
    ...
    public function get_total_bonus($id)
    {
        return $this->db->select('user.id, 
                                    ...
                                    (SELECT sum(bonus.zenx) FROM bonus WHERE bonus.user_id = user.id) sponsorzenx,
                                    (SELECT sum(bonus_sm.zenx) FROM bonus_sm WHERE bonus_sm.user_id = user.id) sponmatchingzenx,
                                    (SELECT sum(bonus_maxmatching.zenx) FROM bonus_maxmatching WHERE bonus_maxmatching.user_id = user.id) pairingmatch,
                                    ...
                                    (SELECT sum(bonus_binarymatch.zenx) FROM bonus_binarymatch WHERE bonus_binarymatch.user_id = user.id) binarymatch,
                                    (SELECT sum(bonus_global.zenx) FROM bonus_global WHERE bonus_global.user_id = user.id) bonusglobal,
                                    (SELECT sum(bonus_basecamp.zenx) FROM bonus_basecamp WHERE bonus_basecamp.user_id = user.id) basecampzenx')
            ->from('user')
            ->where('user.id', $id)
            ->get();
    }
    ...
    {
        return $this->db->select('user.username, user.email, package.name, package.color, cart.datecreate, level_fm.fm, user.country_code')
            ->from('cart')
            ->join('user', 'user.id = cart.user_id')
            ->join('package', 'package.id = cart.package_id')
    ...
    public function row_check_global($user_id, $date)
    {
        return $this->db->select('*')
            ->from('bonus_global')
            ->where(['from_unixtime(datecreate, "%Y-%m") =' => $date, 'user_id' => $user_id])
            ->count_all_results();
    }
    ...
    {
        return $this->db->select('user_id, zenx, COUNT(level_fm) AS total')
            ->from('excess_bonus')
            ->where('level_fm', $level)
    ...
    {
        return $this->db->select('user.username, bonus_basecamp.update_date, bonus_basecamp.cart_id, bonus_basecamp.team, bonus_basecamp.filecoin, bonus_basecamp.zenx, bonus_basecamp.type')
            ->from('bonus_basecamp')
            ->join('cart', 'bonus_basecamp.cart_id = cart.id')
            ->join('user', 'cart.user_id = user.id')
    ...
    {
        return $this->db->select('user.*, level_fm.fm, cart.user_id, cart.sponsor_id, cart.position_id, cart.update_date,
    ...
    {
        return $this->db->select('user.*, level_fm.fm, cart.user_id, cart.sponsor_id, cart.position_id, cart.update_date, SUM(package.point) AS name, SUM(package.price_zenx) AS total_zenx')
            ->from('user')
            ->join('level_fm', 'level_fm.user_id = user.id', 'left outer')
            ->join('cart', 'cart.user_id = user.id', 'left outer')
            ->join('package', 'cart.package_id = package.id', 'left outer')
    ...
    public function totalBonusUser()
    {
        ...
        (SELECT sum(airdrop_zenx.amount) FROM airdrop_zenx) airdrop_zenx,
        (SELECT sum(bonus.filecoin) FROM bonus) sponsorfil, 
        (SELECT sum(bonus.zenx) FROM bonus) sponsorzenx,
        (SELECT sum(bonus_sm.filecoin) FROM bonus_sm) sponmatchingfil, 
        (SELECT sum(bonus_sm.zenx) FROM bonus_sm) sponmatchingzenx,
        (SELECT sum(bonus_maxmatching.zenx) FROM bonus_maxmatching) pairingmatch,
        ...
        (SELECT sum(bonus_binarymatch.zenx) FROM bonus_binarymatch) binarymatch,
        (SELECT sum(bonus_global.zenx) FROM bonus_global) bonusglobal,
        (SELECT sum(bonus_basecamp.filecoin) FROM bonus_basecamp) basecampfill,
        (SELECT sum(bonus_basecamp.zenx) FROM bonus_basecamp) basecampzenx')
            ->from('user')
            ->get();
    }
    ...
    public function totalBonusUserByDate($date)
    {
        ...
        (SELECT sum(airdrop_zenx.amount) FROM airdrop_zenx WHERE from_unixtime(airdrop_zenx.datecreate, "%Y-%m-%d") = "' . $date . '") airdrop_zenx,
        (SELECT sum(bonus.filecoin) FROM bonus WHERE from_unixtime(bonus.datecreate, "%Y-%m-%d") = "' . $date . '") sponsorfil, 
        (SELECT sum(bonus.zenx) FROM bonus WHERE from_unixtime(bonus.datecreate, "%Y-%m-%d") = "' . $date . '") sponsorzenx,
        (SELECT sum(bonus_sm.filecoin) FROM bonus_sm WHERE from_unixtime(bonus_sm.datecreate, "%Y-%m-%d") = "' . $date . '") sponmatchingfil, 
        (SELECT sum(bonus_sm.zenx) FROM bonus_sm WHERE from_unixtime(bonus_sm.datecreate, "%Y-%m-%d") = "' . $date . '") sponmatchingzenx,
        (SELECT sum(bonus_maxmatching.zenx) FROM bonus_maxmatching WHERE from_unixtime(bonus_maxmatching.datecreate, "%Y-%m-%d") = "' . $date . '") pairingmatch,
        ...
        (SELECT sum(bonus_binarymatch.zenx) FROM bonus_binarymatch WHERE from_unixtime(bonus_binarymatch.datecreate, "%Y-%m-%d") = "' . $date . '") binarymatch,
        (SELECT sum(bonus_global.zenx) FROM bonus_global WHERE from_unixtime(bonus_global.datecreate, "%Y-%m-%d") = "' . $date . '") bonusglobal,
        ...
        (SELECT sum(bonus_basecamp.zenx) FROM bonus_basecamp WHERE from_unixtime(bonus_basecamp.datecreate, "%Y-%m-%d") = "' . $date . '") basecampzenx')
            ->from('user')
    ...
    public function allBonusUser($date)
    {
        ...
        (SELECT sum(airdrop_zenx.amount) FROM airdrop_zenx WHERE airdrop_zenx.user_id = user.id AND from_unixtime(airdrop_zenx.datecreate, "%Y-%m-%d") = "' . $date . '") airdrop_zenx,
        (SELECT sum(bonus.filecoin) FROM bonus WHERE bonus.user_id = user.id AND from_unixtime(bonus.datecreate, "%Y-%m-%d") = "' . $date . '") sponsorfil, 
        (SELECT sum(bonus.zenx) FROM bonus WHERE bonus.user_id = user.id AND from_unixtime(bonus.datecreate, "%Y-%m-%d") = "' . $date . '") sponsorzenx,
        (SELECT sum(bonus_sm.filecoin) FROM bonus_sm WHERE bonus_sm.user_id = user.id AND from_unixtime(bonus_sm.datecreate, "%Y-%m-%d") = "' . $date . '") sponmatchingfil, 
        (SELECT sum(bonus_sm.zenx) FROM bonus_sm WHERE bonus_sm.user_id = user.id AND from_unixtime(bonus_sm.datecreate, "%Y-%m-%d") = "' . $date . '") sponmatchingzenx,
    ...
    public function get_last_data($table, $column, $order)
    {
    ...
    {
        return $this->db->select('user.username, package.name, bonus_basecamp.update_date, bonus_basecamp.cart_id, bonus_basecamp.team, bonus_basecamp.zenx, bonus_basecamp.type')
            ->from('bonus_basecamp')
            ->join('cart', 'bonus_basecamp.cart_id = cart.id')
            ->join('package', 'cart.package_id = package.id')
            ->join('user', 'cart.user_id = user.id')
    ...
    public function get_sumzenx_airdrop($user_id)
    {
        return $this->db->select_sum('airdrop_zenx.amount')
            ->from('airdrop_zenx')
            ->join('cart', 'airdrop_zenx.user_id = cart.user_id')
            ->join('package', 'cart.package_id = package.id')
            ->where('airdrop_zenx.user_id', $user_id)
            ->get();
    }
    ...
    {
        return $this->db->select_sum('zenx')
            ->from('excess_bonus')
            ->where('user_id', $id)
            ->get();
    }
    ...
    {
        return $this->db->select('excess_bonus.id, user.username, excess_bonus.zenx, excess_bonus.level_fm, excess_bonus.datecreate')
            ->from('excess_bonus')
    ...
    {
        return $this->db->select('excess_bonus.id, user.username, excess_bonus.zenx, excess_bonus.datecreate')
            ->from('excess_bonus')
            ->join('user', 'user.id = excess_bonus.user_id')
            ->where(['excess_bonus.note' => 'bonus pairing', 'excess_bonus.user_id' => $id, 'excess_bonus.zenx !=' => '0'])
            ->order_by('excess_bonus.datecreate', 'DESC')
            ->get()->result();
    }
    ...
    {
        return $this->db->select('excess_bonus.id, user.username, excess_bonus.generation, excess_bonus.zenx, excess_bonus.datecreate')
            ->from('excess_bonus')
            ->join('user', 'user.id = excess_bonus.user_id')
    ...
    {
        return $this->db->select('user.username, excess_bonus.datecreate, excess_bonus.cart_id, excess_bonus.zenx, basecamp_name.name AS bs_name, package.name')
            ->from('excess_bonus')
            ->join('cart', 'excess_bonus.cart_id = cart.id')
            ->join('user', 'cart.user_id = user.id')
            ->join('package', 'package.id = cart.package_id')
            ->join('basecamp_name', 'basecamp_name.id = excess_bonus.id_bs', 'left')
            ->where(['excess_bonus.user_id' => $id, 'excess_bonus.zenx !=' => '0', 'excess_bonus.note' => 'bonus basecamp'])
            ->order_by('excess_bonus.datecreate', 'DESC')
            ->get()->result();
    }
    ...
    public function get_totalairzenx_bydate($date)
    {
        return $this->db->select_sum('amount')
                        ->from('airdrop_zenx')
                        ->where('from_unixtime(airdrop_zenx.datecreate, "%Y-%m-%d") = ', $date)
                        ->get()->row_array();
    }
    ...
    public function get_basecamp_leader()
    {
        return $this->db->select('basecamp_name.id, basecamp_name.name, user.id as userid, user.username, 
                                ...
                                (SELECT sum(bonus_basecamp.zenx) FROM bonus_basecamp WHERE bonus_basecamp.id_bs = basecamp_name.id and bonus_basecamp.user_id = user.id) bonus, (SELECT sum(excess_bonus.zenx) FROM excess_bonus WHERE excess_bonus.user_id = user.id AND excess_bonus.note = "bonus basecamp") excess')
                        ->from('basecamp_name')
    ...
    {
        return $this->db->select('sum(bonus_basecamp.zenx) as bonus, basecamp_name.name, user.username, user.first_name, level_fm.fm, user.id, (SELECT SUM(package.point) FROM cart JOIN package ON package.id = cart.package_id WHERE cart.user_id = user.id) purchase')
                        ->from('bonus_basecamp')
                        ->join('user', 'user.id = bonus_basecamp.user_id')
                        ->join('level_fm', 'level_fm.user_id = user.id')
    ...
    public function get_omset_bybasecamp($id_bs, $iduser)
    {
        return $this->db->select('bonus_basecamp.datecreate, user.username, basecamp_name.name, (SELECT b.username from cart as a JOIN user as b on b.id = a.user_id WHERE a.id = bonus_basecamp.cart_id) member, (SELECT d.name from cart as c JOIN package as d on d.id = c.package_id WHERE c.id = bonus_basecamp.cart_id) purchase, bonus_basecamp.zenx')
                        ...
                        ->join('user', 'user.id = bonus_basecamp.user_id')
                        ->join('basecamp_name', 'basecamp_name.id = bonus_basecamp.id_bs')
                        ->where(['bonus_basecamp.id_bs' => $id_bs, 'bonus_basecamp.user_id' => $iduser, 'bonus_basecamp.zenx !=' =>'0', 'bonus_basecamp.status' => '1'])
                        ->get()->result();
    }
    ...
    public function get_omset_bybasecamp_gather($id_bs, $iduser)
    {
        return $this->db->select('bonus_basecamp.datecreate, user.username, basecamp_name.name, (SELECT b.username from cart as a JOIN user as b on b.id = a.user_id WHERE a.id = bonus_basecamp.cart_id) member, (SELECT d.name from cart as c JOIN package as d on d.id = c.package_id WHERE c.id = bonus_basecamp.cart_id) purchase, bonus_basecamp.zenx')
                        ...
                        ->join('user', 'user.id = bonus_basecamp.user_id')
                        ->join('basecamp_name', 'basecamp_name.id = bonus_basecamp.id_bs')
                        ->where(['bonus_basecamp.id_bs' => $id_bs, 'bonus_basecamp.user_id' => $iduser, 'bonus_basecamp.zenx !=' =>'0', 'bonus_basecamp.status' => '0'])
                        ->get()->result();
    }

    public function get_omset_bybasecamp_excess($id_bs, $iduser)
    {
        return $this->db->select('excess_bonus.datecreate, user.username, basecamp_name.name, (SELECT b.username from cart as a JOIN user as b on b.id = a.user_id WHERE a.id = excess_bonus.cart_id) member, (SELECT d.name from cart as c JOIN package as d on d.id = c.package_id WHERE c.id = excess_bonus.cart_id) purchase, excess_bonus.zenx')
                        ->from('excess_bonus')
                        ->join('user', 'user.id = excess_bonus.user_id')
                        ->join('basecamp_name', 'basecamp_name.id = excess_bonus.id')
                        ->where(['excess_bonus.id_bs' => $id_bs, 'excess_bonus.user_id' => $iduser, 'excess_bonus.zenx !=' =>'0', 'excess_bonus.note' => 'bonus basecamp'])
                        ->get()->result();
    }
    ...
    public function get_global_fm_bymonth($date, $level)
    {
        return $this->db->select('cart.update_date, user.id, user.username, user.first_name, user.country_code, user.phone, package.name, 
                        ...
                        ->where(['from_unixtime(bonus_global.datecreate, "%Y-%m") =' => $date, 'bonus_global.zenx != ' => '0', 'bonus_global.level_fm' => $level])
                        ->get()->result();
    }
    ...
    public function get_excessglobal_fm_bymonth($date, $level)
    {
        return $this->db->select('cart.update_date, user.id, user.username, user.first_name, user.country_code, user.phone, package.name, 
                        ...
                        ->where(['from_unixtime(excess_bonus.datecreate, "%Y-%m") =' => $date, 'excess_bonus.zenx !=' => '0', 'excess_bonus.level_fm' => $level, 'excess_bonus.note' => 'bonus global'])
                        ->get()->result();
    }
